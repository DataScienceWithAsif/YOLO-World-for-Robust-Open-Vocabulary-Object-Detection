...
@MODELS.register_module()
class YOLOWorldImprovedDetector(BaseModel):
    """
    Detector with fine-grained prompt expansion
    """

    def __init__(self, backbone, init_cfg=None):
        super().__init__(init_cfg)
        self.backbone = backbone

        self.prompt_map = {
            "doctor": [
                "medical doctor wearing white coat",
                "physician examining patient"
            ],
            "nurse": [
                "hospital nurse wearing scrubs",
                "nurse assisting patients in hospital"
            ],
            "police": [
    ...
    def expand_prompt(self, class_name):
        """
        Expand fine-grained classes using descriptive prompts
        """
        return self.prompt_map.get(class_name, [class_name])

    def forward(self, class_name):
        """
        class_name: string
        """
        prompts = self.expand_prompt(class_name)
        embedding = self.backbone(prompts)

        confidence = torch.norm(embedding, dim=-1)
        confidence = confidence / confidence.max()

        return embedding, confidence

# Example usage:
# backbone = MultiPromptBackbone()
# detector = YOLOWorldImprovedDetector(backbone)
# embedding, confidence = detector("doctor")
