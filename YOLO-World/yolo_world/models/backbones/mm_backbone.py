...
@MODELS.register_module()
class MultiPromptBackbone(BaseModule):
    """
    Backbone with prompt ensemble encoding
    Reduces sensitivity to wording variations
    """
    ...
    def encode_prompt_ensemble(self, prompts):
        """
        Encode multiple prompt variants and average embeddings
        """
        embeddings = []
        ...
        for prompt in prompts:
            ...
            with torch.no_grad():
                emb = self.text_model(**inputs).last_hidden_state.mean(dim=1)
                embeddings.append(emb)

        return torch.mean(torch.stack(embeddings), dim=0)

    def forward(self, prompt_variants):
        """
        prompt_variants: list of strings
        """
        return self.encode_prompt_ensemble(prompt_variants)

# Example usage:
# prompts = ["a photo of a dog", "a picture of a canine"]
# backbone = MultiPromptBackbone()
# embedding = backbone(prompts)
